...
evalin('base','set(fig_2Dslice, ''Name'',[field_name,'' R-T P='',num2str(pval),''  Frame: '',num2str(id,''%03d'')],''NumberTitle'',''off'',''Color'',[0.0 0.0 0.0]);');
...
if(draw_cut_lines==1)
  ...
  evalin('base','X_line = squeeze(fliplr(X_cut(tidx,:)''));');
  evalin('base','Y_line = squeeze(fliplr(Y_cut(tidx,:)''));');
  evalin('base','axes(axis_slice);');
  evalin('base','line(Y_line,X_line,''Color'',''w'',''LineWidth'',1.5);');
  evalin('base','plot(axis_slice,Y_line(ridx),X_line(ridx),''wo'',''LineWidth'',1,''MarkerSize'',5,''MarkerFaceColor'',''c'');');
