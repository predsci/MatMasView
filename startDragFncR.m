...
%   evalin('base','[X_cut Y_cut] = pol2cart(squeeze(T(pidx,:,:)),squeeze(R(pidx,:,:)));');
    ...
    evalin('base','set(fig_2Dslice, ''Name'',[field_name,'' P-T R='',num2str(rval),''  Frame: '',num2str(id,''%03d'')],''NumberTitle'',''off'',''Color'',[0.20 0.20 0.25]);');
    ...
    evalin('base','X_line = squeeze(P(pidx,:,ridx));');
    evalin('base','Y_line = squeeze(T(pidx,:,ridx));');
